...
export const Index: React.FC = () => {
  ...
  const { loading: processingTokenEntry, error: tokenEntryError } = useGetUserProfileQuery({
    skip: !token?.raw,
  ...
  const handleTokenChange = React.useCallback((value: string) => {
    try { setToken({ raw: value, payload: jwtDecode(value) });  }
  ...
  const onHeroExited = React.useCallback(() => { setLandingStep(2); }, []);
  const onWelcomeEntered = React.useCallback(() => { setLandingStep(3); }, []);
  const onWelcomeExited = React.useCallback(() => { 
    console.log('Navigating to dashboard');
    navigate({ to: '/dashboard' }); 
  }, [navigate]);

  if ([0, 1].includes(landingStep)) return (
    <Box className={clsx(mbp.page)}>
      <Fade in={landingStep === 0} appear={false} timeout={1000} onExited={onHeroExited}>
        <Hero 
